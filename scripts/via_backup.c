...
static bool ex(IOHIDDeviceRef d,In*in,const uint8_t*req){
    ...
    return in->received && in->result==kIOReturnSuccess && in->buf[0]!=0xff;
}
int main(void){
    ...
    if(!ex(dev,&in,req)){fprintf(stderr,"layer count read failed\n");return 4;}
    int layers=in.buf[1];
    ...
    for(int off=0; off<total; off+=CHUNK){
        ...
        if(!ex(dev,&in,r2)){fprintf(stderr,"buffer read failed at offset %d\n",off);return 5;}
        printf("%04x:",off);

...
让键盘背光实时显示 Claude Code 在干什么：它正在工作时灯是流动的青色，停下来等你批准时
...
- **支持的键盘**：任何支持 VIA 配置协议的 QMK 键盘都能驱动键区背光（QMK 是客制化键盘
  常用的开源固件，很多量产键盘出厂就带 VIA）。上图绕底座一圈的 50 颗环形灯是
  NuPhy Halo65 V2 独有的，需要刷本仓库的固件补丁，也只在这块键盘上真机验证过。
- **连接方式**：USB 数据线即插即用；刷过固件补丁的 Halo65 V2 用蓝牙也行；
  2.4G 接收器不支持。
- 只支持 macOS。
...
需要：macOS、USB 数据线直连的键盘（有些线只能充电，不能传数据）、
Xcode Command Line Tools、系统自带的 `/usr/bin/python3`。

## 长什么样

下文把键区背光称作**内圈**，把 Halo65 V2 底座那圈环形灯称作**外圈**。
状态之间用动画区分，颜色只是强化 —— 用余光看键盘时，「一个点在转圈」和
「整圈一起明灭」分得清，两种不同的红色分不清。
...
外圈默认关闭：原厂固件没有提供从电脑控制这圈灯的接口，必须刷本仓库的固件补丁。
如果默认打开，没刷过补丁的键盘装完只会看起来像坏了。

多个会话同时跑时取优先级最高的：**语音输入 > 工具失败 > 等待权限 > 执行中 > 全部完成 >
默认**。语音输入排在最前，因为其余状态说的是后台在做什么，它说的是你此刻正在做什么。

「全部完成」是一次性提示：被任何新状态打断后不再恢复，没被打断时倒计时结束熄灭。
倒计时和失败保护窗的时长都在设置 App 对应状态的页面里调节。细节在
[AGENTS](docs/AGENTS.md)。

## 语音输入

按下你自己的语音输入快捷键时，灯变成另一套效果，松开就回到当时该显示的状态。默认关闭，
在设置 App 的「语音输入」页打开。
...
留一个无效的开关**。蓝牙下探测不到，已开启的外圈不会被它关掉。

它同时会打开 Orca 桥接。Orca 是一个管理多台机器上终端会话的工具：hook 只能看到本机的
会话，你在其他机器上跑的 Claude Code 由这个桥接轮询 Orca 读回来，接进同一套灯。
没在用 Orca 的话，这部分与你无关。

外圈需要自己编译固件，步骤和工具链的注意事项在
[`firmware/README.md`](firmware/README.md)。**进 DFU 刷机模式会清空键盘存储，
先备份改键**（`build/via_backup`）。
...
- **概览** —— 当前显示的状态和环形动画、各状态的会话数、连接方式（USB / 蓝牙）、
  外圈可控性、hooks 安装情况。
- **每个状态一页** —— 选动画、取色、调节亮度速度拖尾，左边有 50 点环形实时预览，
...
| `voice.tail_seconds` | 触发结束后再保持多久，盖过语音转文字的延迟 |
...
- **内圈（键区背光）开箱即用**：任何经 USB 直连、支持 VIA 且实现了 RGB Matrix
  的 QMK 键盘都能驱动。键盘是按 VIA 接口自动发现的，不固定厂商和产品 ID。
  跑一次 `scan`，它会直接告诉你这块板子有什么可用。
- **外圈和蓝牙通道来自同一份固件补丁**，补丁只针对 NuPhy Halo65 V2 写成。
  别的键盘不能直接刷，但可以移植：`halo_link.[ch]` 是纯协议解码器，不含板级代码，
  可以整体搬走；动画实现和 VIA 通道处理写在 Halo65 V2 的 `side.c` / `ansi.c` 里，
  要按你的板子的灯珠排布重写。每个文件的改动内容见
...
Orca 桥接（默认关闭）会读远程终端的标题和预览来判断状态，这些内容同样不落盘。

## 排查

| 症状 | 先做什么 |
| --- | --- |
| 灯光完全没反应 | 跑 `scan`。有线下多半是 VIA / NuPhy 官方软件独占了键盘接口，或线只能充电；蓝牙下要刷过固件补丁并给 `halo65_leds` 授权（`install.py leds`）；2.4G 不支持 |
| 只有键区亮，四边不亮 | 这是默认行为。跑 `reconnect` |
| `hooks: 0/7` | 重跑 `install`，然后 `Command + Q` 完全退出 Claude Code |
| 灯光停在某个颜色不动 | agent 异常退出没发 `Stop`。`send-event SessionEnd` |
...
- 只支持 macOS。扫描对任何 QMK/VIA 键盘有效，内圈原理上也通用，但**只在 NuPhy Halo65 V2
  （`19f5:3315`）上真机验证过**。
- 外圈需要自己编译固件；官方固件更新会覆盖它，需要重刷。50 颗灯作为一个整体显示状态。
- 蓝牙依赖同一份固件补丁，且改配置走每秒约 16 bit 的低速通道：状态切换零点几秒，
  改一种颜色约 8 秒，一次改动太多会提示插线补全。键盘睡眠期间灯光不更新。

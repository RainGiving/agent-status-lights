...
Claude Code 或 Codex 都行。[`docs/AI-INSTALL.md`](docs/AI-INSTALL.md) 是写给 agent 的
runbook：部署 → 扫描 → 按扫描结果分支 → （可选）刷外圈固件 → 安装服务和 App → 验收，
每一步都带判据和失败时该说什么。

**有一步它替你做不了。** 进 DFU 刷固件必须由人按住 `Esc` 的同时插上 USB 线，
没有任何命令能替代 —— 所以 runbook 走到那里会**停下来等你**，前后的备份和恢复
（改键备份、原厂固件 dump、刷完写回改键）它自动做。

另外，**大多数人根本不需要那一步**：内圈在原厂固件上就能用，runbook 默认也不会推销
刷机，只有在你的键盘确实是 Halo75 V2、并且你主动说想要外圈时才会提。

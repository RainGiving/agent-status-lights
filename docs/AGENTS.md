...
README 给了结论，这里是细节和各自的边界：多个会话怎么合并成一盏灯、Codex 少什么、
Orca 桥接怎么工作、蓝牙通道的分工。
...
```text
语音输入 > 失败 > 等待权限 > 执行中 > 全部完成 > 默认
```

所以只要有**任何一个**会话要你批准就会看到琥珀脉冲；而绿色只在**所有**会话都结束时出现
（它在活跃状态里优先级最低，任何一个还在跑都会覆盖它）。App 的「概览」页会列出每个状态
各有几个会话。

「全部完成」是一次性提示，被打断后不会恢复：绿色被任何更高优先级状态覆盖时，对应的完成
记录随即丢弃，那个状态结束后灯回到默认而不是回到绿色 —— 完成已经被看到，之后的灯只反映
新的事。远程轮询来源的会话例外：它们由轮询器每个周期重新上报，何时结束由轮询器自己决定。

被强杀的 Claude Code 不会发出 `SessionEnd`，那个会话会一直停在「执行中」。所以活跃状态
...
保持 4 秒后，下一次工具调用就会把它恢复成黄色。想让红色留得更久，在设置 App
「工具失败」页把保护窗调大即可。
...
`session_id` 和 `hook_event_name` —— 而这两个字段正好就是本项目唯一会读的两个（其余全部丢弃，
...
守护进程只留两个字段，这条隐私边界比一个红灯更重要，所以没做。

两边的会话共用同一个聚合器，按 `session_id` 区分（都是 UUID，不会冲突），优先级规则照旧
—— 一个 Codex 在跑、一个 Claude 在等你批准，你看到的是琥珀色。

> ⚠️ 这条路径是**照 Codex 的 hooks 文档写的，没有在真机上跑过** —— 写这段时本机没装
> Codex。安装器会像 Claude Code 那条路径一样先自测一次 hook 命令（用 `/bin/sh` 实跑、
> 检查退出码和 stdout），但事件是否真的按预期触发，需要你装上 Codex 之后用
> `install.py status` 的 `codex:` 那一行和 `daemon.log` 确认。

Orca 桥接那条路径是 agent 无关的：它根据终端标题里的 spinner 字形判断，所以远程跑 Codex
...
灯是正常的；你在远程机器上敲 claude，灯不会有任何反应。

Orca 会在它管理的每台主机上装自己的 hook，所以这些会话它看得见。`src/orca_bridge.py`
就是去轮询 `orca` CLI 把它们读回来，作为伪会话（键名 `orca:<scope>:<handle>`）并入同一个
优先级聚合器。本机会话仍然走 hook —— 轮询器**故意跳过**它们，免得用粗粒度状态覆盖掉细的。

它默认关着，除了「原厂能不能用」之外还有一个理由：轮询每隔两秒多启动一批子进程，去读每台
...
放权限标记（Orca 那个 `✋` 是 Gemini CLI 的），所以「等待权限」只能检索 `preview` 末尾的
提示词 —— 措辞会随版本变，因此做成了配置项而不是固定的正则。
...
Orca 没开或 CLI 不在时，桥接只是退避重试，本机路径完全不受影响。

## 第三条输入：语音输入

前两条输入都在说后台在做什么。`halo65_voice` 说的是**你此刻正在做什么**，所以
`voice` 状态排在优先级最前面：正在口述的时候，覆盖一个还在转圈的彗星是对的。

它是一个独立的启动项 `com.claudehalo65.voice`，不是由守护进程启动的子进程。原因是
「输入监控」这项权限记在**发起请求的那个进程**上：由 Python 守护进程 fork 出来的话，
权限会记到 `/usr/bin/python3` 名下，既授权给了整个 Python，也得不到本项目自己的身份。
...
事件监听只做一件事：把每个按键事件的键码和四个修饰键与配置的那一组比对，命中就向
守护进程的 socket 发送 `{"source": "voice", "active": true/false}`。**不上报按了什么键**，
...
有线走 VIA raw HID，主机随时下发颜色和动画参数。蓝牙不暴露 VIA，主机唯一能推给
键盘的是标准键盘 LED 报文，RF 模块只把它的低 3 位经串口转给主控，其中大写锁定位
归系统所有。可用的只有 NumLock 和 ScrollLock 两位，净速率按符号自时钟编码算约
16 bit/s。完整协议、时序阈值和仿真测量在 [PROTOCOL](PROTOCOL.md) 的
「wireless status link」一节。

对使用者来说，**App 里只有一套菜单，插不插线都用同一份配置**。差别收敛成三条：

| | 有线（USB） | 蓝牙 |
| --- | --- | --- |
| 状态切换 | 即时 | 常用三态约 0.2 秒，其余约 0.3–0.8 秒 |
| 改配置 | 即时生效 | 松手后经低速通道同步，改一种颜色约 8 秒，App 显示进度 |
| 预览 | 预览当前编辑值 | 预览键盘里已存的参数，同步完成前可能是旧值 |

分工是三层：守护进程只决定「现在是什么状态、配置改了哪些字段」，写进
`led.state`；启动项 `halo65_leds` 负责线路时序（周期重发、转义对、配置帧），它和
`halo65_voice` 一样需要单独的「输入监控」授权（`install.py leds`），因为往键盘写
LED 位就要打开键盘设备；固件里的 `halo_link.c` 解码状态号，从 EEPROM 的六个状态
槽里取参数渲染，动画本来就跑在固件里。插线时守护进程用 VIA 把六个槽一次写全并
存 EEPROM，所以无线会话平时只传 2 位状态号；无线下改配置才走慢速帧，一次改动太多
（比如恢复默认）会截断到 9 帧并提示插线补全。

已知边界：

- 键盘睡眠后灯光随之停止，唤醒后约 0.3 秒内由周期重发恢复。
- 系统的大写键报文会把两个数据位清零，解码器按大写位的跳变识别这次覆盖并忽略，
  实测（仿真 30 组）不会覆盖状态灯；正在传配置帧时一次大写按键至多损失一份
  副本，三份连发里剩下两份仍能满足「连续两份一致」。
- 2.4G 接收器没有 VIA 也不转发这条通道之外的任何主机数据，已放弃，插 2.4G 时
  等同于「未连接」。
- 蓝牙多通道切换到另一台主机后，本机的状态自然不再送达；切回来后由周期重发恢复。

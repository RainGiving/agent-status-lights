...
（它在活跃状态里优先级最低，任何一个还在跑都会压住它）。App 的「概览」页会列出每个状态
各有几个会话。

「全部完成」是一次性提示，被打断后不会恢复：绿色被任何更高优先级状态覆盖时，对应的完成
记录随即丢弃，那个状态结束后灯回到默认而不是回到绿色 —— 完成已经被看到，之后的灯只反映
新的事。远程轮询来源的会话例外：它们由轮询器每个周期重新断言，何时结束由轮询器自己决定。
...
权限会记到 `/usr/bin/python3` 名下，既授权给了整个 Python，也拿不到本项目自己的身份。
...
- 2.4G 接收器没有 VIA 也不转发这条通道之外的任何主机数据，已放弃，插 2.4G 时
  等同于「未连接」。
- 蓝牙多通道切换到另一台主机后，本机的状态自然不再送达；切回来靠周期重发恢复。
